...
> 编校注（非原文）：原书此处写作 `gethostbyname`；结合图 12-38 中 `ctime_ts` 调用 `ctime` 的代码，此处疑应为 `ctime`，非转写错误。
...
![图 12-48 一个有死锁的程序的进度图](assets/fig-12-48.jpg)
...
![图 12-49 改正后的无死锁的程序的进度图](assets/fig-12-49.jpg)
