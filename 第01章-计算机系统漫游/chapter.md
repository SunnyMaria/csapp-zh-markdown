...
[返回总目录](../README.md) · [上一章](../前言/README.md) · [下一章](../第02章-信息的表示和处理/README.md) · [按小节阅读](README.md) · [练习题答案](练习题答案.md)
...
大部分的现代计算机系统都使用 ASCII 标准来表示文本字符，这种方式实际上就是用一个唯一的单字节大小的整数值<sup>①</sup>来表示每个字符。比如，图 1-2 中给出了 hello.c 程序的 ASCII 码表示。
...
> **C 编程语言的起源**
>
> C 语言是贝尔实验室的 Dennis Ritchie 于 1969 年～1973 年间创建的。美国国家标准学会（American National Standards Institute，ANSI）在 1989 年颁布了 ANSI C 的标准，后来 C 语言的标准化成了国际标准化组织（International Standards Organization，ISO）的责任。这些标准定义了 C 语言和一系列函数库，即所谓的 C 标准库。Kernighan 和 Ritchie 在他们的经典著作中描述了 ANSI C，这本著作被人们满怀感情地称为“K&R”[61]。用 Ritchie 的话来说[92]，C 语言是“古怪的、有缺陷的，但同时也是一个巨大的成功”。为什么会成功呢？
>
...
> **注** 有其他编码方式用于表示非英语类语言文本。具体讨论参见 2.1.4 节的旁注。
...
![图 1-3 编译系统](1.2/assets/fig-1-3-compilation-system.png)
...
> **GNU 项目**
>
> GCC 是 GNU（GNU 是 GNU's Not Unix 的缩写）项目开发出来的众多有用工具之一。GNU 项目是 1984 年由 Richard Stallman 发起的一个免税的慈善项目。该项目的目标非常宏大，就是开发出一个完整的类 Unix 的系统，其源代码能够不受限制地被修改和传播。GNU 项目已经开发出了一个包含 Unix 操作系统的所有主要部件的环境，但内核除外，内核是由 Linux 项目独立发展而来的。GNU 环境包括 EMACS 编辑器、GCC 编译器、GDB 调试器、汇编器、链接器、处理二进制文件的工具以及其他一些部件。
>
> GCC 编译器已经发展到支持许多不同的语言，能够为许多不同的机器生成代码。支持的语言包括 C、C++、Fortran、Java、Pascal、面向对象 C 语言（Objective-C）和 Ada。
>
...
![图 1-4 一个典型系统的硬件组成](1.4/assets/fig-1-4-typical-system-hardware.png)
...
![图 1-5 从键盘上读取 hello 命令](1.4/assets/fig-1-5-reading-hello-command.png)
...
![图 1-6 从磁盘加载可执行文件到主存](1.4/assets/fig-1-6-loading-executable.png)
...
![图 1-7 将输出字符串从存储器写到显示器](1.4/assets/fig-1-7-writing-output.png)
...
![图 1-8 高速缓存存储器](1.5/assets/fig-1-8-cache-memory.png)
...
![图 1-9 一个存储器层次结构的示例](1.6/assets/fig-1-9-memory-hierarchy.png)
...
![计算机系统的分层视图](1.7/assets/fig-1-10-layered-view.png)
...
![操作系统提供的抽象表示](1.7/assets/fig-1-11-operating-system-abstractions.png)
...
![进程的上下文切换](1.7/assets/fig-1-12-process-context-switch.png)
...
![进程的虚拟地址空间](1.7/assets/fig-1-13-process-virtual-address-space.png)
...
![网络也是一种 I/O 设备](1.8/assets/fig-1-14-network-as-io-device.png)
...
![利用 telnet 通过网络远程运行 hello](1.8/assets/fig-1-15-remote-hello-over-telnet.png)
...
![不同的处理器配置分类](1.9/assets/fig-1-16-processor-configurations.png)
...
![多核处理器的组织结构](1.9/assets/fig-1-17-multicore-processor-organization.png)
...
![计算机系统提供的一些抽象](1.9/assets/fig-1-18-computer-system-abstractions.png)
