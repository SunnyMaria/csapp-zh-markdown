...
[返回总目录](../README.md) · [上一章](../第07章-链接/README.md) · [下一章](../第09章-虚拟内存/README.md) · [按小节阅读](README.md) · [练习题答案](练习题答案.md)
...
- [第 8 章 异常控制流](#第-8-章-异常控制流)
  ...
  - [8.4 进程控制](#84-进程控制)
    ...
    - [8.4.6 利用 `fork` 和 `execve` 运行程序](#846-利用-fork-和-execve-运行程序)
  - [8.5 信号](#85-信号)
    - [8.5.1 信号术语](#851-信号术语)
    - [8.5.2 发送信号](#852-发送信号)
    - [8.5.3 接收信号](#853-接收信号)
...
其中，每个 a<sub>k</sub> 是某个相应的指令 I<sub>k</sub> 的地址。每次从 a<sub>k</sub> 到 a<sub>k+1</sub> 的过渡称为控制转移（control transfer）。这样的控制转移序列叫做处理器的控制流（flow of control 或 control flow）。

最简单的一种控制流是一个“平滑的”序列，其中每个 I<sub>k</sub> 和 I<sub>k+1</sub> 在内存中都是相邻的。这种平滑流的突变（也就是 I<sub>k+1</sub> 与 I<sub>k</sub> 不相邻）通常是由诸如跳转、调用和返回这样一些熟悉的程序指令造成的。这样一些指令都是必要的机制，使得程序能够对由程序变量表示的内部程序状态中的变化做出反应。
...
![异常的剖析](8.1/assets/fig-8-1-anatomy-of-an-exception.jpg)
...
![异常表](8.1/assets/fig-8-2-exception-table.jpg)
...
![生成异常处理程序的地址](8.1/assets/fig-8-3-generating-exception-handler-address.jpg)
...
![中断处理](8.1/assets/fig-8-5-interrupt-handling.jpg)
...
![陷阱处理](8.1/assets/fig-8-6-trap-handling.jpg)
...
![故障处理](8.1/assets/fig-8-7-fault-handling.jpg)
...
![终止处理](8.1/assets/fig-8-8-abort-handling.jpg)
...
![逻辑控制流](8.2/assets/fig-8-12-logical-control-flows.jpg)
...
![进程地址空间](8.2/assets/fig-8-13-process-address-space.jpg)
...
![进程上下文切换的剖析](8.2/assets/fig-8-14-process-context-switch.jpg)
...
![图 8-15 中示例程序的进程图](8.4/assets/fig-8-16-fork-process-graph.jpg)
...
![嵌套 fork 的进程图](8.4/assets/fig-8-17-nested-fork-process-graph.jpg)
...
![一个新程序开始时用户栈的典型组织结构](8.4/assets/fig-8-22-initial-user-stack.jpg)
...
![信号处理](8.5/assets/fig-8-27-signal-handling.png)
...
![前台和后台进程组](8.5/assets/fig-8-28-process-groups.png)
...
![信号处理程序可以被其他信号处理程序中断](8.5/assets/fig-8-31-nested-signal-handlers.png)
