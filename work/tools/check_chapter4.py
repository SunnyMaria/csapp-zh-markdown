...
ROOT = Path("output/csapp-faithful/chapter4")
CONTENT_DIRS = [
    ROOT / "4.1",
    ROOT / "4.2",
    ROOT / "4.3",
...
def count_heading(text: str, label: str, number: int) -> int:
    pattern = rf"\*\*{re.escape(label)} 4\.{number}(?!\d)"
...
def main() -> int:
    ...
    for number in range(1, 45):
        ...
        if count != 1:
            issues.append(f"练习题 4.{number}: expected once, found {count}")

    for number in range(45, 60):
        count = count_heading(all_text, "家庭作业", number)
        if count != 1:
            issues.append(f"家庭作业 4.{number}: expected once, found {count}")

    for number in range(1, 71):
        caption = rf"^(?!\!\[)\s*\**图 4-{number}(?!\d)"
        if not re.search(caption, all_text, re.MULTILINE):
            issues.append(f"图 4-{number}: caption missing")
    ...
    print(
        ...
        "exercises 4.1-4.44; homework 4.45-4.59; figures 4-1-4-70"
    )
    return 0
