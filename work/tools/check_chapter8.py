...
ROOT = Path("output/csapp-faithful/chapter8")
CONTENT_DIRS = [ROOT / f"8.{number}" for number in range(1, 9)] + [
    ROOT / "homework"
]


def count_label(text: str, label: str, number: int) -> int:
    pattern = rf"\*\*{re.escape(label)} 8\.{number}(?!\d)"
...
def main() -> int:
    ...
    for number in range(1, 9):
        ...
        if count != 1:
            issues.append(f"练习题 8.{number}: expected once, found {count}")

    for number in range(9, 27):
        count = count_label(all_text, "家庭作业", number)
        if count != 1:
            issues.append(f"家庭作业 8.{number}: expected once, found {count}")

    for number in range(1, 47):
        caption = rf"^\s*(?:>\s*)?\*\*图 8-{number}\*\*"
        count = len(re.findall(caption, all_text, re.MULTILINE))
        if count != 1:
            issues.append(f"图 8-{number}: expected one caption, found {count}")
    ...
    print(
        ...
        "exercises 8.1-8.8; homework 8.9-8.26; figures 8-1-8-46"
    )
    return 0
